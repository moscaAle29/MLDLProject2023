        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(self.test_loader):
                ...
                if i % 50 == 0:
                    ...
                    images = Image.fromarray(images.cpu().unsqueeze(dim = 1).numpy())
                    prediction = Image.fromarray(prediction.cpu().unsqueeze(dim = 1).numpy())
                    labels = Image.fromarray(labels.cpu().unsqueeze(dim = 1).numpy())
